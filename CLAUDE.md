...
- **Org-switch (how Adam moves between the two Suttons)**: only superusers (Abahou) get the org dropdown in the top-right nav. Pick "Team Sunshine Construction" to work inside the friend's org (all tabs show their data); pick "Ventana Heating and Cooling" to return. The switch lives in the login session — logging out and back in always lands you in your own org. Every switch (both directions) writes an `OrgSwitchAudit` row (user, from/to org, IP, timestamp), viewable at `/org/audit/`. Data created while switched belongs to the org you're switched into.
- **Current state**: Ventana is empty (no leads/reps/configs yet) — an empty CRM after Adam logs in is correct. All pre-2026-08-03 data belongs to Team Sunshine. Do not modify Team Sunshine's rows without explicit authorization.
...
- **Neither Twilio number both delivers SMS and hears replies.** The 978 delivers, but its inbound routes to MarketingCanvas, not `/sms/`. The 833's inbound is `/sms/`, but its outbound SMS is disabled (toll-free registration never completed). Interim: Ventana sends from the 978, and Team Sunshine from the 833, whose texts are not delivered. Never put Team Sunshine on the 978 (Ventana's A2P registration). See INTEGRATION.md §6.4.
